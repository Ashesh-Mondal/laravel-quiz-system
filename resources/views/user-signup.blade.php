...
<body class="bg-gray-100">
    ...
    <div class=" flex justify-center items-center min-h-screen">
        <div class="bg-white p-8 rounded-2xl shadow-lg w-full max-w-sm">
            <h1 class="text-2xl mb-5 text-center">User Signup Page</h1>
            @error('user')
                <span class="inline-block text-red-600">{{ $message }}</span>
            @enderror
            <form action="{{ route('user.signup') }}" method="post" class="space-y-2">
                @csrf
                <div>
                ...
                <div class="flex justify-center items-center">
                    <button type="submit"
                        class="py-2 rounded-xl bg-blue-300 text-blue-900 hover:bg-blue-800 hover:text-white cursor-pointer mt-4 w-full transition-all ease-in-out duration-300">SignUp</button>
                </div>
            </form>
        </div>
    </div>
